...
--esta tabla contiene los modelos de partida que puede haber, con todos sus parÃ¡metros de 
...
insert into tiposCasillas values (7,'castigar') --no se define la casilla donde se sufre el castigo, serÃ¡ un estado del jugador
...
(
  ...
  estado int not null, --estado de la partida, indica si estÃ¡ creada(1) o iniciada (2). No s eespecifica el 3 - finalizada, porque se eliminara
  tablero int not null references tableros(id)
)
...
fecha: 20210927
registra un usuario, se asume que la contraseÃ±a ya viene encriptada
*/
...
Autor: Alberto Botana
fecha: 20210927
autentica un usuario. Simplemente valida que el usuario es vÃ¡lido o no. SerÃ­a responsabilidad de la aplicaciÃ³n 
garantizar que el usuario sigue siendo el mismo durante toda la sesiÃ³n, pero no se va a implementar
*/
...
Autor: Alberto Botana
fecha: 20210928
descripcion: aÃ±ade un jugador a la partida. Si el idusuario es ulo, entonces serÃ¡ un bot
*/
...
Autor: Alberto Botana
fecha: 20210928
descripciÃ³n: actualiza el nivel de construccion de un determinado tipo que tiene un jugador
...
Autor: Alberto Botana
fecha: 20210928
descripciÃ³n: compra la casilla en la que estÃ¡ el jugador. Se valida que la casilla no este vendida
*/
...
Autor: Alberto Botana
fecha: 20210928
descripciÃ³n: vende a casilla indicada y recalcula posibles edificaciones. Se valida que se apropiedad del jugador
*/
